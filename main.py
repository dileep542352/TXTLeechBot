...
auth_users = [6882767118,6882767118]
sudo_users = auth_users
sudo_groups = [-1002340573597]
        ...
        for i in range(arg, len(links)):
            try:
                ...
                try:
                    ...
                    await m.reply_text(f"Failed to get video info for {url}")
                    continue

                # Resolution selection logic
                ytf = None
                if raw_text2 in ["144", "180", "240", "360", "480", "720"]:
                    available_qualities = {
                        "144": ["256x144", "320x180"],
                        "180": ["320x180", "426x240"],
                        "240": ["426x240", "426x234", "480x270", "480x272", "640x360"],
                        "360": ["640x360", "638x360", "636x360", "768x432"],
                        "480": ["854x480", "852x480", "854x470", "768x432"],
                        "720": ["1280x720", "1280x704", "1280x474", "1920x712"]
                    }
                    
                    for quality in available_qualities[raw_text2]:
                        if quality in out:
                            ytf = out[quality]
                            break
                    
                    if ytf is None:
                        ytf = out.get("unknown", list(out.values())[0] if out else None)
                    ...
                    ytf = quality_map.get(raw_text2, "http-360p")
                else:
                    ytf = list(out.values())[0] if out else None

                if ytf is None:
                    await m.reply_text(f"No suitable quality found for {url}")
                    continue

                # Get resolution for naming
                try:
                    res = "NA" if "unknown" in out else list(out.keys())[list(out.values()).index(ytf)]
                except Exception:
                    res = "NA"
